...
```sh
git clone https://github.com/natalie-o-perret/go-sql.git
cd go-sql
```
...
```text
feat: add QueryAll to query package
fix(scan): handle nil scan target for unknown columns
docs: update README transaction example
refactor(query): simplify WithTx error handling
```
